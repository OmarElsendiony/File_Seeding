    ...
    for service_name, service_data in structure.items():
        for subfolder, subfolder_data in service_data.items():
            for filename in subfolder_data.get("files", []):
                ...
                # Only process .py entries from the JSON
                if not filename.endswith(".py"):
                    continue

                # Logical key stays .py
                logical_key = f"{subfolder}/{filename}"

                # Replace .py → .txt for actual file read
                txt_filename = filename[:-3] + ".txt"

                real_path = root_dir / service_name / subfolder / txt_filename
...
build_files_content_json(
    ...
    root_dir=".",
    output_json_path="files_fixes.json"
)
